...
5. **team-lead가 README.md 갱신 (reviewer 보고 수신 후 직접 수행)**
   - `README.md`를 읽고 레퍼런스 문서 수정 사항 중 README에 반영이 필요한 항목 파악
   - 반영 기준:
     - 파일 경로·패키지 구조 변경 → `## 프로젝트 구조` 섹션
     - 기술 스택 버전·라이브러리 추가/제거 → `## 기술 스택` 표
     - 라우트 추가/제거 → `## Renderer 구조` 라우트 표
     - 아키텍처·데이터 흐름 변경 → `## 아키텍처 개요` 다이어그램
     - 상세 문서 목록 변경(파일 추가/제거) → `## 상세 문서` 표
   - README는 **외부 공개용**이므로 내부 레퍼런스 세부 내용(코딩 규칙, 상수값 등)은 추가하지 않는다
   - 변경이 없으면 "README 변경 없음" 기록 후 넘어간다
6. team-lead가 최종 결과 출력 (레퍼런스 문서 수정 N건 + README 변경 여부 포함)
