...
**1디렉토리 N에이전트**: `dirToAgentIds: Map<string, Set<number>>` — 동일 프로젝트 디렉토리에 복수 에이전트 허용. 디렉토리 이벤트는 해당 Set의 에이전트 모두에 라우팅.
...
~/.claude/plugins/installed_plugins.json
  { version: 2, plugins: { "claude-studio@local": [{ scope, installPath, version, installedAt, lastUpdated }] } }
    │
    ▼
...
`isPluginInstalled()` — `~/.claude/plugins/installed_plugins.json`에 `"claude-studio@local"` 키 존재 여부로 판단.
