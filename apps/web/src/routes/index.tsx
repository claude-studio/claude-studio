...
import { formatCost, formatCostUsd, formatTokens, formatNumber, timeAgo, formatDate } from '@repo/shared';
...
function OverviewPage() {
  ...
  const activeDays = stats.dailyUsage.filter(d => d.messages > 0);
  const firstDay = activeDays.length > 0 ? formatDate(new Date(activeDays[0]!.date)) : null;
  const lastDay = activeDays.length > 0 ? formatDate(new Date(activeDays[activeDays.length - 1]!.date)) : null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          description={firstDay && lastDay ? `${firstDay} ~ ${lastDay}` : undefined}
