        ...
        <CardContent>
          <div className="space-y-3">
            {[...session.messages].reverse().map((msg, i) => {
              ...
              const msgCost =
                typeof raw.costUSD === 'number' && raw.costUSD > 0 ? (raw.costUSD as number) : null;
              ...
              return (
                <div key={i} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                  ...
                  >
                    ...
                    >
                      ...
                      {msgCost !== null && (
                        <span className="text-[10px] text-muted-foreground/70 font-mono">
                          {formatCost(msgCost)}{' '}
                          <span className="opacity-70">({formatCostUsd(msgCost)})</span>
                        </span>
                      )}
                    </div>
                    <div
                    ...
                    >
                      {isTruncated && !isExpanded ? text.slice(0, 500) + '...' : text}
                    </div>
                    {isTruncated && (
                      <button
                        onClick={() => toggleExpand(i)}
                        className="text-[11px] text-muted-foreground hover:text-foreground transition-colors self-start ml-1"
                      >
                        {isExpanded ? '접기 ↑' : `더 보기 (${text.length - 500}자 더) ↓`}
                      </button>
                    )}
                  </div>
                </div>
