...
import { formatCost, formatCostUsd, formatTokens, formatDateShort } from '@repo/shared';
...
type PeriodFilter = 'all' | '30days' | 'month' | 'week';
...
function filterByPeriod(data: DailyUsage[], period: PeriodFilter): DailyUsage[] {
  ...
  if (period === '30days') {
    ...
    const from = toLocalDateStr(past);
...
function CostsPage() {
  ...
  const [period, setPeriod] = React.useState<PeriodFilter>('30days');
  ...
  const dailyAvg = dayOfMonth > 0 ? thisMonthCost / dayOfMonth : 0;
  const forecastCost = thisMonthCost + dailyAvg * remainingDays;
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-border/50">
          <CardContent className="p-4">
            ...
            <p className="text-xl font-semibold mt-1"><CostDisplay cost={thisMonthCost} /></p>
        ...
        <Card className="border-border/50">
          <CardContent className="p-4">
            ...
            <p className="text-xl font-semibold mt-1"><CostDisplay cost={lastMonthCost} /></p>
        ...
        <Card className="border-border/50 border-dashed">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-muted-foreground/60">일평균 × 잔여일</span>
            </div>
            <p className="text-xl font-semibold mt-1 text-primary"><CostDisplay cost={forecastCost} /></p>
            <p className="text-xs text-muted-foreground mt-1">잔여 {remainingDays}일, 일평균 {formatCost(dailyAvg)} <span className="opacity-70">({formatCostUsd(dailyAvg)})</span></p>
          </CardContent>
        </Card>
      </div>
