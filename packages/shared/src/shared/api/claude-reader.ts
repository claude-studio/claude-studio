...
export function getTeams(claudeDir?: string): import('../types/index').TeamDetail[] {
  const teamsDir = path.join(claudeDir ?? getClaudeDir(), 'teams');
  const tasksDir = path.join(claudeDir ?? getClaudeDir(), 'tasks');
  if (!fs.existsSync(teamsDir)) return [];

  const results: import('../types/index').TeamDetail[] = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        // Read tasks
        const teamTasksDir = path.join(tasksDir, entry);
        const tasks: import('../types/index').TaskInfo[] = [];
