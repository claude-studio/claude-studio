    ...
    <TooltipProvider delayDuration={300}>
      <div className="space-y-3">
        ...
        <div className="flex items-end justify-between gap-3 pb-3 border-b border-border/40">
          <div>
            <FieldLabel label={t('cache.labels.hitRate')} tooltip={t('cache.tooltips.hitRate')} />
            <p className="text-3xl font-bold font-mono mt-1 tabular-nums">{data.cacheHitRate}%</p>
          </div>
          <div className="text-right">
            <FieldLabel
              label={t('cache.labels.estimatedSavings')}
              tooltip={t('cache.tooltips.estimatedSavings')}
            />
            <p className="text-xl font-semibold font-mono mt-1 text-primary tabular-nums">
              {formatCost(data.estimatedSavingsUsd, { locale })}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {t('cache.labels.hit')} {data.cacheHitRate}%
            </span>
            <span className="flex items-center gap-1">
              {t('cache.labels.miss')} {missRate}%
        ...
        <div className="space-y-2 pt-1">
          <div className="flex items-center justify-between">
            <FieldLabel
              label={t('cache.labels.readTokens')}
              tooltip={t('cache.tooltips.readTokens')}
            />
            <p className="text-sm font-semibold font-mono tabular-nums text-primary">
              {formatTokens(data.totalCacheReadTokens, { locale })}
            </p>
          </div>
          <div className="flex items-center justify-between">
            <FieldLabel
              label={t('cache.labels.creationTokens')}
              tooltip={t('cache.tooltips.creationTokens')}
            />
            <p className="text-sm font-medium font-mono tabular-nums text-muted-foreground">
              {formatTokens(data.totalCacheCreationTokens, { locale })}
            </p>
          </div>
        </div>
      </div>
